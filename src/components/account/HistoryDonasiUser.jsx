    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {historyDonasiById.map((donasi) => (
        <Link
          ...
          className="flex flex-col justify-between bg-white border hover:bg-green-50 border-green-300 rounded-lg shadow-md p-5  transition-all h-full"
        >
          {/* Judul dan lokasi sumbangan */}
          <div className="mb-3">
            <h2 className="text-xl font-semibold text-green-900 truncate first-letter:uppercase">
              {donasi.sumbangan?.title ?? "Tidak ada judul"}
            </h2>
            <p className="text-sm text-green-700 truncate first-letter:uppercase leading-tight mt-1">
              {donasi.sumbangan?.location ?? "Lokasi tidak tersedia"}
            </p>
          </div>

          {/* Daftar item donasi */}
          <div className="mb-4 max-h-28 overflow-y-auto rounded border border-green-100  p-3">
